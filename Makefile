...
Qt_DEFINES 	= -DQT_NO_DEBUG -DQT_WEBKIT_LIB -DQT_GUI_LIB -DQT_NETWORK_LIB -DQT_CORE_LIB -DQT_SHARED
Qt_CFLAGS  	= -pipe -O2 -Wall -W -D_REENTRANT $(Qt_DEFINES)
Qt_CXXFLAGS	= -pipe -O2 -Wall -W -D_REENTRANT $(Qt_DEFINES)
Qt_INCS 		= -I/usr/share/qt4/mkspecs/linux-g++ \
							...
							-I/usr/include/qt4 \
							-I.
Qt_LIBS			= -L/usr/lib -lQtWebKit -lQtGui -lQtNetwork -lQtCore -lpthread 

OBJS := har_to_pagespeed.o ssps.o
$(OBJS): CFLAGS := $(CFLAGS_$(BUILDTYPE)) $(CFLAGS_C_$(BUILDTYPE)) $(DEFS_$(BUILDTYPE)) $(INCS_$(BUILDTYPE)) $(Qt_CFLAGS) $(Qt_INCS)
$(OBJS): CXXFLAGS := $(CFLAGS_$(BUILDTYPE)) $(CFLAGS_CC_$(BUILDTYPE)) $(DEFS_$(BUILDTYPE)) $(INCS_$(BUILDTYPE)) $(Qt_CXXFLAGS) $(Qt_INCS)
har_to_pagespeed.o: $(pagespeed_root_dir)/pagespeed/samples/har_to_pagespeed.cc
	$(CXX) $(CXXFLAGS) -c -o $@ $<
ssps.moc:
	/usr/bin/moc-qt4 $(Qt_DEFINES) $(Qt_INCS) ssps.h -o ssps.moc
ssps.o: ssps.cc ssps.moc
	$(CXX) $(CXXFLAGS) -c -o $@ $<
...
ssps: LDFLAGS := $(LDFLAGS_$(BUILDTYPE))
ssps: $(PAGESPEED_DEPS) ssps.o
	$(LINK) $(LDFLAGS) -o $@ -Wl,--start-group $(PAGESPEED_DEPS) $^ -Wl,--end-group $(LIBS)

clean:
	rm -rf har_to_pagespeed har_to_pagespeed.o ssps ssps.o $(pagespeed_root_dir)/$(pagespeed_builddir)

all: har_to_pagespeed ssps
